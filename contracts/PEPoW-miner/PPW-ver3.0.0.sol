...
contract PPWToken is ERC20, ReentrancyGuard {
    struct MinerData {
    ...
    constructor(uint256 initialSupply, uint256 _miningDifficulty, uint256 _blockReward, uint256 _maxStakeSize) ERC20("PEPOW Miner", "PPW") {
        require(initialSupply > 1000000, "Initial supply must be greater than zero");
        require(_miningDifficulty > 1, "Mining difficulty must be greater than zero");
        require(_blockReward > 10, "Block reward must be greater than zero");
        require(_maxStakeSize > 50000, "Max stake size must be greater than zero");
   ...
   function mine(uint256 nonce) public nonReentrant {
    ...
    function resumeMining() external {
